...
| | 在哪里调 | 在哪配 | 换了要重烧吗 |
|---|---|---|---|
| **LLM** | 设备直接调 | 固件 `main/chat_config.h` | 要 |
| **ASR / TTS** | 后端调 | 后端 `server/.env` | 不用，重启后端即可 |

之所以这么分：LLM 是纯文本、一次往返，ESP32-C3 扛得住 TLS；音频是流式的、
数据量大、各家协议五花八门，放在后端灵活得多。

配置都在编译期/启动期定好，**设备上的配网页只管 Wi-Fi**。

---

## 一、换 LLM

设备直接请求任何 **OpenAI 兼容**的 `/chat/completions`。改 `main/chat_config.h`：

```c
#define CHAT_LLM_BASE_URL "https://api.deepseek.com/v1"   // 到 /v1 为止
#define CHAT_LLM_API_KEY  "sk-你自己的"
...
| 栏位 | 注意 |
|---|---|
| `BASE_URL` | 填到 `/v1` 为止，**不要**带 `/chat/completions`；末尾斜杠会自动去掉 |
| `API_KEY` | 会**编进固件镜像**，所以别把编好的 `.bin` 发给别人 |
| `MODEL` | 服务商的模型标识 |

没填（还是模板里的占位值）的话，设备上会直接提示"还没配 LLM"，
而不是拿假 key 去撞一个 401。
...
- **必须支持流式**（`stream: true`）。设备靠增量逐句送去合成，非流式会退化成
  "等全部说完才出声"。
- **换密钥要重新烧录**。嫌麻烦的话，可以自己把这三项挪回 NVS + 配网页
  （历史上就是那么做的，`chat_provision.c` 里加三个表单字段即可），
  但那样 key 就要在手机上输入。
